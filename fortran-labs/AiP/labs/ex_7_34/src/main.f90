! Задание ex_7_34
! В матрице B(10,15) поменять местами диагональный и максимальный элементы в каждой строке.
! Указания
! На входе матрица квадратная. Сперва сформировать массив из номеров максимальных элементов в каждой строке (MaxLoc с параметром dim), 
! чтобы эта операция могла быть векторизована. 
! В цикле с независимыми итерациями и условием (do concurrent с условием) 
! провести замену диагонального элемента, если его номер в строке отличается от максимального. 
program exercise_7_34
   ...
   real(R_), allocatable, target :: A(:, :) 
   real(R_), pointer             :: mainDiag(:) 
   real(R_), allocatable         :: maxElemsA(:), A1(:)
   ! i - строка, j - столбец
